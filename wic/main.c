...
HINSTANCE instance;
char *PluginName = "wic";
char *PluginDescription = "wParam's Infernal Chronometer";
...
plugfunc_t PluginFunctions[] = 
{
	...
	{"wic.test", "v", WicTestFunction, "Tests", "", ""},
	{"wic.debugprint", "ic", WicSetDebugPrint, "Gets the current value of the debug prints flag", "(wic.debugprint) = [i:val]", "Normally, errors are handled but not output.  If the debug prints value is nonzero, information about errors will be printed to the p5 console.  Use this to help diagnose problems with WIC"},
	{"wic.debugprint", "ici", WicSetDebugPrint, "Sets the current value of the debug prints flag", "(wic.debugprint [i:newval]) = [i:oldval]", "Normally, errors are handled but not output.  If the debug prints value is nonzero, information about errors will be printed to the p5 console.  Use this to help diagnose problems with WIC.  This function returns the previous value."},

	{"wic.create", "hesiiii", WicCreate, "", "", ""},
	{"wic.destroy", "vei", WicDestroy, "", "", ""},

	{"wic.add.text", "veiiissi", TxtAddLabel, "", "", ""},
...
void PluginInit (parerror_t *pe)
{
	...
	WicSentinel = GetTickCount ();

	wc.cbWndExtra = sizeof (void *);
	wc.hbrBackground = GetStockObject (BLACK_BRUSH);
	wc.hCursor = LoadCursor (NULL, IDC_ARROW);
	wc.hIcon = LoadIcon (instance, MAKEINTRESOURCE (IDI_WIC));
	wc.hInstance = instance;
	wc.lpfnWndProc = WicProc;
	wc.lpszClassName = WIC_CLASS;
...
{
	UnregisterClass (WIC_CLASS, instance);
